...
if __name__ == "__main__":
    ...
    data_by_devices = []
    for selection in selections:
        for document in db.collection("data_by_sensor").where("sessionId", "==", sessions[int(selection)]["sessionId"]).order_by("binId").stream():
        ...
        for i in range(len(data_by_devices)):
            ...
            movements = db.collection("sessions").document(session_id).get().to_dict()["movements"]
            movement_data = dict()
            for movement in movements:
                movement_data[movement["name"]] = movement["startTime"].astimezone()

            device = db.collection("devices").document(device_id).get()

            file_name += device_type + "-" + device_id + "-" + session_id + ".csv"
            ...
            for mvmt in movement_data.keys():
                section = float((movement_data[mvmt] - start).total_seconds()) * 20
                if section < 0:
                    section = 0
                plt.axvline(x = section, color = 'b')
                plt.text(section+5, 2500, mvmt)
