...
static void ble_nus_c_evt_handler(ble_nus_c_t * p_ble_nus_c, ble_nus_c_evt_t const * p_ble_nus_evt)
{
	...
	switch (p_ble_nus_evt->evt_type)
	{
		case BLE_NUS_C_EVT_DISCOVERY_COMPLETE:
		  ...
		  printf("\r\Connection established! ADC started.\r\n");
			break;
	...
	if (p_event->type == NRF_DRV_SAADC_EVT_DONE)
	{
		...
		{ // Take the average of each channel, convert to resistance, send serial data
			for (uint32_t i = 0; i < NUM_ADC_CHANS * MUX_NUM_CHANNNELS; i++)
			{
				...
				char stringResistance[NUM_BYTES_TO_SEND];

				if (i < MUX_LAST_CHANNEL)
				{
					//printf("%.1f,", i, sensorResistance);
					sprintf(stringResistance, "%d", i); //"%.1f,", sensorResistance);
					ble_nus_c_string_send(&m_ble_nus_c, (uint8_t *)stringResistance, (uint16_t)strlen(stringResistance));
