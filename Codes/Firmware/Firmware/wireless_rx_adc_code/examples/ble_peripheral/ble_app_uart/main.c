...
#define MAX_CONN_INTERVAL               MSEC_TO_UNITS(10.0, UNIT_1_25_MS)             /**< Maximum acceptable connection interval (75 ms), Connection interval uses 1.25 ms units. */
...
static void nus_data_handler(ble_nus_t * p_nus, uint8_t * p_data, uint16_t length)
{
    switch (state)
    {
        case RX_STANDBY: // Waiting for the first message chunk
        {
            ...
            {
								//printf("RX_STDBY: received:%c%d, ", p_data[0], (int)p_data[1]);
								strncpy(serString, "", sizeof(serString));
                state = RX_TRANSMISSION;
								//numm = (marker > '9') ? (marker &~ 0x20) - 'A' + 10: (marker - '0');
                numMsgs = (int)p_data[1];
                strncpy(serString, p_data+2, length-2); // first three symbols are part of the header
								msgIndex = 2;
            }
						break;
        }
        case RX_TRANSMISSION: // Collecting further message chunks
        {
            if (p_data[0] == '!') // first symbol in the last chunk, marks the end of the sequence
            {
                ...
                strncpy(serString + strlen(serString), p_data+2, length-2); // ignore 2 symbols at the front and '!'
                sprintf(serString + strlen(serString), "\r\n");
                printf("%s", serString);
                state = RX_STANDBY;
            }
            else if (p_data[0] == ':') // marks the current message index
            {
                if ((int)p_data[1] == msgIndex)
                {
                    strncpy(serString + strlen(serString), p_data+2, length-2); // ignore 2 symbols in the header
                    msgIndex++;
                }
                else
								{
