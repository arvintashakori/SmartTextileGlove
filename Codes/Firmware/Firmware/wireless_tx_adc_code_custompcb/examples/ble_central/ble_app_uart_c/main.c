...
float sensResBuf[NUM_ADC_CHANS * MUX_NUM_CHANNNELS] = {0};
...
static void buttons_leds_init(void)
{
	...
	//err_code = bsp_btn_ble_init(NULL, &startup_event);    COMMENTED THIS OUT ******
	//APP_ERROR_CHECK(err_code);														COMMENTED THIS OUT ******
}
...
void saadc_sampling_event_init(void)
{
	...
	err_code = nrf_drv_timer_init(&p_timer, &timer_cfg2, timer_handler);
	APP_ERROR_CHECK(err_code);*/
...
void saadc_callback(nrf_drv_saadc_evt_t const * p_event)
{
	if (p_event->type == NRF_DRV_SAADC_EVT_DONE)
	{
		...
		/*capture*/
		//uint32_t sensorResistanceCalculationTime = nrf_drv_timer_capture(&p_timer, NRF_TIMER_CC_CHANNEL0);
		//nrf_drv_timer_disable(&p_timer);
		//float timeusec = time * 0.0625;
		...
		{	
			...
			{ // Take the average of each channel, convert to resistance, send serial data
				...
				// uint32_t floatToStringTime = nrf_drv_timer_capture(&p_timer, NRF_TIMER_CC_CHANNEL0);
				// nrf_drv_timer_disable(&p_timer);
				
				
				uint32_t numPackets = (strlen(stringResistance) / MAX_MSG_PAYLOAD_SIZE_BYTES) + 1;
				for (int currPacketIndex = 1; currPacketIndex <= numPackets; currPacketIndex++)
				{
					...
					if (currPacketIndex == 1)
						sprintf(strTemp,"?%c",(char)numPackets);
					else if (currPacketIndex < numPackets)
						sprintf(strTemp,":%c", (char)currPacketIndex);
					else
					{
						sprintf(strTemp,"!%c", (char)currPacketIndex);
					...
					strncpy(strTemp+2, //TODO 
							stringResistance + MAX_MSG_PAYLOAD_SIZE_BYTES*(currPacketIndex - 1),
							numBytesToSend);
					
					
					// uint32_t creating1Packet = nrf_drv_timer_capture(&p_timer, NRF_TIMER_CC_CHANNEL0);
					// nrf_drv_timer_disable(&p_timer);
					//float timeusec = time * 0.0625;

					// Send chunks of 20 bytes unless this is a final chunk, which can have a smaller size
					
					uint32_t numTimesSkipped = 0;
					//nrf_drv_timer_enable(&p_timer);
					do{
						err_code = ble_nus_c_string_send(&m_ble_nus_c, 
										 (strTemp), 
										 (uint16_t)numBytesToSend+2);
					}while(err_code == NRF_ERROR_RESOURCES);

					strncpy(strTemp, "", sizeof(strTemp));
					// uint32_t sending1Packet = nrf_drv_timer_capture(&p_timer, NRF_TIMER_CC_CHANNEL0);
					// nrf_drv_timer_disable(&p_timer);
